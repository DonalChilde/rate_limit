"""conftest.py file for rate_limit"""
import json
import logging
...
from rate_limit.app_lib.logging import rotating_file_logger
...
def json_resources_example_(logger: logging.Logger) -> Dict[str, FileResource]:
    ...
    """
    json_resources = {}
    resource_path: str = "tests.rate_limit.resources.example.json_resources"
...
def env_setup(monkeypatch, test_app_data_dir):
    ...
    monkeypatch.setenv(
        "PFMSOFT_RATE_LIMIT_TESTING",
        "True",
    )
    monkeypatch.setenv(
        "PFMSOFT_RATE_LIMIT_LOG_LEVEL",
        str(APP_LOG_LEVEL),
    )
    monkeypatch.setenv(
        "PFMSOFT_RATE_LIMIT_APP_DIR",
